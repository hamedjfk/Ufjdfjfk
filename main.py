...
from telegram.ext import (
    Application, ChatJoinRequestHandler, ChatMemberHandler, CommandHandler, ContextTypes,
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if owner(update):
        text = (
            '✅ ربات فعال است.\n\n'
            'راهنمای راه‌اندازی (فقط شما به‌عنوان مالک):\n\n'
            '1️⃣ ربات را در گروه عمومی ادمین کنید و مجوز حذف پیام بدهید.\n'
            '2️⃣ داخل گروه عمومی بفرستید:\n'
            '/setup public\n\n'
            '3️⃣ ربات را در گروه VIP ادمین کنید و مجوز تأیید درخواست عضویت بدهید.\n'
            '4️⃣ داخل گروه VIP بفرستید:\n'
            '/setup vip لینک_درخواست_عضویت_VIP\n\n'
            'مثال:\n'
            '/setup vip https://t.me/+LINK\n\n'
            'فرمان‌ها:\n'
            '/setcount 10 — تغییر تعداد اد لازم\n'
            '/status — وضعیت ربات\n'
            '/backup — ارسال بکاپ فوری\n\n'
            '📦 بکاپ خودکار هر ۳۰ دقیقه به همین پی‌وی ارسال می‌شود.\n'
            '♻️ برای ریستور، فایل JSON بکاپ را با کپشن /restore بفرستید.'
        )
    else:
        text = '✅ ربات فعال است. برای عضویت در VIP، قوانین گروه عمومی را انجام دهید.'
...
async def restore(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_chat.type != 'private' or not message.document:
        ...
        return
    # هر فایل JSON که مالک در پی‌وی ربات بفرستد، خودکار ریستور می‌شود.
    # کپشن لازم نیست.
...
async def count_direct_add(context: ContextTypes.DEFAULT_TYPE, chat_id: int, inviter, member, reply_to=None):
    """فقط افزودن مستقیم یک نفر توسط شخص دیگر را ثبت می‌کند؛ با شناسه عضو از دوباره‌شماری جلوگیری می‌شود."""
    if chat_id != db['publicChatId'] or not inviter or inviter.is_bot or member.is_bot:
        return
    # اگر فرد خودش با لینک وارد شود، from_user همان خودش است و اد محسوب نمی‌شود.
    if member.id == inviter.id or str(member.id) in db['countedMembers']:
        return
    ...
    await save_db()
    log.info('اد ثبت شد: inviter=%s member=%s total=%s', inviter.id, member.id, item['count'])
    if item['qualified'] and not db['notifiedQualified'].get(str(inviter.id)) and db['vipInviteLink']:
        ...
        if reply_to:
            await reply_to.reply_html(text)
        else:
...
async def new_members(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for member in message.new_chat_members:
        await count_direct_add(context, update.effective_chat.id, message.from_user, member, message)


async def chat_member_update(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """راه دومِ تلگرام برای تشخیص اضافه‌شدن عضو؛ برای بعضی گروه‌ها از پیام سرویس مطمئن‌تر است."""
    change = update.chat_member
    if not change or change.chat.id != db['publicChatId']:
        return
    old_status = change.old_chat_member.status
    new_status = change.new_chat_member.status
    became_member = old_status in ('left', 'kicked') and new_status in ('member', 'administrator', 'restricted')
    if became_member:
        await count_direct_add(context, change.chat.id, change.from_user, change.new_chat_member.user)
...
def main():
    ...
    # سندهای JSON در پی‌وی مالک؛ خود تابع کپشن /restore را بررسی می‌کند.
    ...
    # علاوه بر پیام سرویس، رویداد رسمی تغییر وضعیت عضو هم بررسی می‌شود.
